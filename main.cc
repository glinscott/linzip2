...
private:
  ...
  int matchPrice(int len, int dist) {
    return 3 + log2(std::max(1, len)) + log2(std::max(1, dist));
  }
  ...
  int64_t writeFSEStream(uint8_t* out, int max_symbols,
...
std::unique_ptr<uint8_t> readFile(const char* path, int64_t& len) {
  FILE* f = fopen(path, "r");
  CHECK(f != nullptr);
  fseek(f, 0, SEEK_END);
  len = ftell(f);
  ...
  CHECK(fread(buf.get(), 1, len, f) == size_t(len));
  fclose(f);
  return std::move(buf);
}
...
void compressFile(const char* path) {
  int64_t len;
  std::unique_ptr<uint8_t> input = readFile(path, len);
  ...
  double compress_elapsed = timer.elapsed() / 1000;
  printf("Encoded %lld into %lld bytes\n", len, encoded_size);
  printf("Compression %.2lf seconds, %.2lf MB/s\n",
         compress_elapsed,
         (len / (1024. * 1024.)) / compress_elapsed);

  std::unique_ptr<uint8_t> decoded(new uint8_t[len]);
  timer = Timer();
  lzDecompress(out.get(), encoded_size, decoded.get(), len);
  double decompress_elapsed = timer.elapsed() / 1000;
  printf("Decompression %.2lf seconds, %.2lf MB/s\n",
         decompress_elapsed,
         (len / (1024. * 1024.)) / decompress_elapsed);

  CHECK(checkBytes(decoded.get(), buf, len) == 0);
  printf("Decompression validated!\n");
}

#ifndef LINZIP_LIBRARY_ONLY
int main(int argc, char const* argv[]) {
  const char* input_path = argc > 1 ? argv[1] : kDefaultInputPath;
  compressFile(input_path);
  return 0;
}
#endif
